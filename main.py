            ...
            if success:
                ...
                for i in range(10):
                    ...
                    if cred_manager.last_updated > old_timestamp:
                        time_diff = cred_manager.last_updated - old_timestamp
                        print(f"✅ 无头模式: 凭证已更新 (耗时 {time_diff:.1f}秒)")
                        _refresh_fail_count = 0
                        # 关键：主动通知所有等待者刷新已完成
                        cred_manager.refresh_event.set()
                        cred_manager.refresh_complete_event.set()
                        print("   🔔 已通知所有等待者凭证刷新完成")
                        return  # 成功，直接返回
...
async def main():
    ...
    # 初始化API密钥管理器
    if not api_key_manager.load_keys():
        print("⚠️ API密钥加载失败，将使用无认证模式")
    else:
        print(f"🔑 已加载 {len(api_key_manager.api_keys)} 个API密钥")
