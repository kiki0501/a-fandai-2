...
class StreamProcessor:
    ...
    def __init__(self, enable_heartbeat: bool = True, heartbeat_interval: float = 15.0):
        ...
        self._api_error_occurred = False  # 追踪是否发生API错误
        self._api_error_message = ""  # 存储API错误消息
    ...
    def process_vertex_response(
        ...
        if 'error' in data:
            ...
            self._log_debug(f"Vertex AI错误: {error_msg}")
            self._stats["errors"] += 1
            self._api_error_occurred = True
            self._api_error_message = str(error_msg)
            return
        ...
        for path_index, result in indexed_results:
            ...
            if 'errors' in result:
                for err in result['errors']:
                    ...
                    # 检查资源耗尽错误
                    if "Resource exhausted" in msg or "exhausted" in msg.lower():
                        raise ResourceExhaustedError(f"Resource exhausted: {msg}")
    ...
    async def process_stream(
        ...
        try:
            ...
            async for chunk in response_iterator:
                ...
                # data_str_1 = str(json_objects)

                # pattern = r"'text':([\s\S]*?), 'thought'"
                # matches = re.findall(pattern, data_str_1)
                # self._log_debug(f"{matches}")
            ...
            if is_error:
                ...
                if not content_yielded:
                    # 如果发生了API错误，抛出异常返回500
                    if self._api_error_occurred:
                        self._log_debug("⚠️ API错误且无内容输出，抛出异常")
                        raise APIError(f"API Error: {self._api_error_message}")
                    
                    # 正常情况但无内容输出，抛出异常返回505（上游空回复）
                    self._log_debug("⚠️ 无内容输出（正常），抛出上游空回复异常")
                    raise EmptyResponseError("Upstream returned empty response")
