...
from src.core import TokenStatsManager, CredentialManager, MODELS_CONFIG_FILE, MAX_CONCURRENT_REQUESTS
from src.stream import get_stream_processor, AuthError as StreamAuthError, ResourceExhaustedError
...
class VertexAIClient:
    ...
    def __init__(self, cred_manager: CredentialManager, stats_manager: TokenStatsManager,
        ...
        # Semaphore 限流：限制同时进行的上游请求数
        self._semaphore = asyncio.Semaphore(MAX_CONCURRENT_REQUESTS)
        self._queue_count = 0  # 当前等待队列中的请求数
        self._active_count = 0  # 当前正在处理的请求数
        self._queue_lock = asyncio.Lock()  # 保护队列计数器的锁
        
        print(f"🔒 Semaphore 限流已启用: 最大并发 {MAX_CONCURRENT_REQUESTS}")
        ...
        # 构建代理配置
        proxy_settings = {}
        if config.get('proxy', {}).get('enable_socks5'):
            proxy_url = f"socks5://{config['proxy']['socks5_host']}:{config['proxy']['socks5_port']}"
    ...
    async def stream_chat(self, messages: List[Dict[str, str]], model: str, **kwargs):
        ...
        async with self._queue_lock:
            ...
            if queue_position > 1 or self._active_count >= MAX_CONCURRENT_REQUESTS:
                print(f"⏳ 请求排队中 (队列位置: {queue_position}, 活跃: {self._active_count}/{MAX_CONCURRENT_REQUESTS})")
        
        # 标记是否已经获取了semaphore（用于正确处理取消）
        acquired_semaphore = False
        
        try:
            # 获取 Semaphore（阻塞直到有空位）
            async with self._semaphore:
                ...
                async with self._queue_lock:
                    ...
                    print(f"🚀 请求开始处理 (活跃: {self._active_count}/{MAX_CONCURRENT_REQUESTS}, 队列: {self._queue_count})")
                
                try:
                    # 执行实际的流式聊天逻辑
                    async for chunk in self._stream_chat_inner(messages, model, **kwargs):
                        yield chunk
                finally:
                    # 完成时减少活跃计数
                    async with self._queue_lock:
                        self._active_count -= 1
                        print(f"✅ 请求完成 (活跃: {self._active_count}/{MAX_CONCURRENT_REQUESTS}, 队列: {self._queue_count})")
        except asyncio.CancelledError:
            # 客户端取消请求时，更新队列状态
            async with self._queue_lock:
                # 只有在还在队列等待时（未获取semaphore）才减少队列计数
                # 如果已经获取semaphore，队列计数已经在上面减过了
                if not acquired_semaphore and self._queue_count > 0:
                    self._queue_count -= 1
                print(f"❌ 请求已取消 (队列: {self._queue_count}, 活跃: {self._active_count})")
            raise
    
    async def _stream_chat_inner(self, messages: List[Dict[str, str]], model: str, **kwargs):
        """实际的流式聊天逻辑（内部方法）"""
        max_retries = 2
        ...
        for attempt in range(max_retries + 1):
                ...
                async with self.cred_manager.refresh_lock:
                    ...
                    if not self.cred_manager.latest_harvest:
                        ...
                        if attempt == 0:
                            ...
                            print(f"⚠️ 凭证已过期，重试刷新 (尝试 {attempt+1}/{max_retries+1})...")
                        should_refresh = True
                    ...
                    if should_refresh:
                        ...
                        if refreshed:
                            ...
                            if ui_ready:
                                ...
                                print("⚠️ 前端未确认刷新完成，但凭证已更新，继续重试...")
                            await asyncio.sleep(1)
                        elif not refreshed and not self.cred_manager.latest_harvest:
                            if attempt == max_retries:
                                error_msg = "⚠️ **Proxy Error**: Could not refresh credentials after multiple attempts.\n\nPlease ensure **Google Vertex AI Studio** is open in your browser and the Harvester script is active."
                                chunk = {
                                    "id": "error-no-creds",
            ...
            try:
                ...
                # 加载配置
                config = load_config()
                disable_thinking = config.get('disable_thinking_mode', False)
                ...
                try:
                    ...
                    async with isolated_client.stream('POST', url, headers=headers, json=new_body) as response:
                        ...
                        if response.status_code != 200:
                            ...
                            print(f"✗ API 错误: {response.status_code} - {error_text_str}")
                            
                            # Check for potential token expiration or permission error
                            if (response.status_code in [400, 401, 403] or "The caller does not have permission" in error_text_str) and attempt < max_retries:
                                print(f"⚠️ 认证或权限错误，刷新中...")
                        ...
                        if stream_error:
                            # 检查是否为资源耗尽错误
                            if isinstance(stream_error, ResourceExhaustedError):
                                if content_yielded:
                                    # 已发送内容，无法重试
                                    print("⚠️ 已发送内容，无法重试")
                                    error_payload = {"error": {"message": f"Resource exhausted mid-stream: {str(stream_error)}", "type": "resource_error"}}
                                    yield f"data: {json.dumps(error_payload)}\n\n"
                                    yield "data: [DONE]\n\n"
                                    return
                                ...
                                if attempt < max_retries:
                                    ...
                                    print(f"⏳ 资源耗尽，等待{wait_time}秒后重试 (尝试 {attempt+1}/{max_retries+1})...")
                                    await asyncio.sleep(wait_time)  # 资源耗尽时等待
                                    continue  # 重试循环
                                
                                # 重试用尽 - 返回错误信息
                                error_msg = "Resource has been exhausted.Please try again later."
                                error_payload = {"error": {"message": error_msg, "type": "resource_error"}}
                                yield f"data: {json.dumps(error_payload)}\n\n"
                                yield "data: [DONE]\n\n"
                                return
                            ...
                            # 重试用尽或刷新失败 - 返回友好错误信息
                            error_msg = "Authentication failed after multiple attempts.Please try again later."
                    ...
                    if content_yielded:
                        print("⚠️ 已发送内容，无法重试")
                        error_payload = {"error": {"message": f"Resource exhausted mid-stream: {str(e)}", "type": "resource_error"}}
                        yield f"data: {json.dumps(error_payload)}\n\n"
                        return
                    ...
                    error_payload = {"error": {"message": "Resource has been exhausted.Please try again later.", "type": "resource_error"}}
                    yield f"data: {json.dumps(error_payload)}\n\n"
                    return
